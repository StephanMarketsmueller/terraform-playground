...
resource "aws_launch_configuration" "example-config" {
  image_id        = "ami-00d5e377dd7fad751"
  instance_type   = "t2.micro"
  security_groups = [aws_security_group.instance.id]
  user_data       = data.template_file.user_data.rendered

  # Needed as launch cofigurations are immutable and this is referenced by the ASG
  lifecycle {
    create_before_destroy = true
  }
}

resource "aws_autoscaling_group" "example-group" {
  launch_configuration = aws_launch_configuration.example-config.id
